...
const startServer = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        server = app_1.default.listen(env_1.envVars.PORT, () => {
            console.log(`server listening to port ${env_1.envVars.PORT} ✅`);
        });
    }
    catch (error) {
        console.log(error);
...
process.on("unhandledRejection", (err) => {
    console.log(`unHandle Rejection dedacted. Server Shutting Down... \n error is:${err}`);
    if (server) {
        server.close(() => {
...
process.on("uncaughtException", (err) => {
    console.log(`uncaught Exception Dedected. Server Shutting Down... \n error is:${err}`);
    if (server) {
        server.close(() => {
            process.exit(1);
        });
